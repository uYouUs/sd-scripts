    ...
    def get_image_size(self, image_path):
        ...
        if image_size[0] <= 0:
            ...
            try:
                ...
                print(f"failed to get image size: {image_path}, error: {e}")
                image_size = (0, 0)
        return image_size
...
class DreamBoothDataset(BaseDataset):
    ...
    def __init__(
        ...
        print(f"{num_train_images} {images_split_name} images with repeats.")

        self.num_train_images = num_train_images

        print(f"{num_reg_images} reg images with repeats.")
        ...
        for subset in subsets:
            ...
            if os.path.exists(subset.metadata_file):
                if subset.metadata_file.endswith(".jsonl"):
                    print(f"loading existing JSOL metadata: {subset.metadata_file}")
            ...
            if size_set_from_metadata > 0:
                print(f"set image size from metadata: {size_set_from_metadata}/{len(image_keys_sorted_by_length_desc)}")
            if num_filtered > 0:
                print(f"filtered {num_filtered} images by original resolution from {subset.metadata_file}")
...
class ControlNetDataset(BaseDataset):
    def __init__(
        ...
        if len(extra_imgs) > 0:
            print(f"extra conditioning data for {len(extra_imgs)} images / 余分な制御用画像があります: {extra_imgs}")
